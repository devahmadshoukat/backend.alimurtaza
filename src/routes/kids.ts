...
router.post("/trending/:productId", auth, async (req, res) => {
  try {
    ...
    if (product.category.gender !== "kids") {
      return handleError(res, { message: "Product must be from kids category" });
    }
...
router.delete("/trending/:productId", auth, async (req, res) => {
  try {
    const { productId } = req.params;
...
router.get("/status", async (req, res) => {
  try {
    ...
    const trendingCount = await Product.countDocuments({
      isActive: true,
      isTrending: true,
      "category.gender": "kids"
    });
